...
// ─── Floating orbit tag ──────────────────────────────────────────
function OrbitTag({ label, baseX, baseY, color = '#c8f135' }: { label: string; baseX: number; baseY: number; color?: string }) {
  const ref = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (!rect) { raf.current = requestAnimationFrame(tick); return }
      const cx = rect.left + rect.width / 2, cy = rect.top + rect.height / 2
...
function Nav() {
  ...
  return (
    <nav style={{
      ...
      <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 16, color: '#c8f135', letterSpacing: '-0.02em', cursor: 'none' }}>
        atabo.philip
      </div>
...
function Hero() {
  const tags = [
    { label: 'REACT', baseX: -230, baseY: -90, color: '#7DF9FF' },
    { label: 'NODE.JS', baseX: 200, baseY: -110, color: '#c8f135' },
    { label: 'LINUX', baseX: -190, baseY: 100, color: '#FF6FD8' },
    { label: 'DOCKER', baseX: 220, baseY: 80, color: '#c8f135' },
    { label: 'DNS', baseX: -50, baseY: -150, color: '#FF6FD8' },
    { label: 'WORDPRESS', baseX: 60, baseY: 150, color: '#7DF9FF' },
  ]
  ...
  return (
    <section data-hero id="hero" style={{
      ...
      <div style={{ textAlign: 'center', position: 'relative', zIndex: 1, padding: '0 24px' }}>
        <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 11, letterSpacing: '0.3em', color: '#c8f135', marginBottom: 28, opacity: 0.8 }}>
          FULL STACK · IT INFRASTRUCTURE · ABUJA, NIGERIA
        </div>
        <h1 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 'clamp(48px, 9vw, 120px)', lineHeight: 0.88, letterSpacing: '-0.03em', color: '#f0f0f0', margin: '0 0 16px' }}>
          ATABO
          <br />
      ...
      <div style={{ position: 'relative', zIndex: 2, width: 1, height: 1, marginTop: 80 }} data-hero>
        {tags.map((t, i) => <OrbitTag key={i} {...t} />)}
        <Magnetic strength={0.55} radius={210}>
          <a
            ...
            style={{
              ...
              padding: '20px 48px',
              fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 15, letterSpacing: '0.15em',
      ...
      {/* Sidebar stats */}
      <div style={{ position: 'absolute', left: 32, bottom: 40, display: 'flex', flexDirection: 'column', gap: 16 }}>
        {[['6+', 'years combined'], ['250+', 'users supported'], ['3', 'platforms shipped']].map(([v, l]) => (
          <div key={l}>
...
function Skills() {
  ...
  return (
    <section id="skills" style={{ padding: '0 40px 120px', maxWidth: 1100, margin: '0 auto' }}>
...
function Projects() {
  ...
  return (
    <section id="projects" style={{ padding: '0 40px 120px', maxWidth: 1100, margin: '0 auto' }}>
      <SectionLabel n="03" text="PROJECTS" />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: '#151515' }}>
        {projects.map(p => (
          <div key={p.n} style={{ background: '#080808', padding: '40px 40px', display: 'grid', gridTemplateColumns: '64px 1fr auto', gap: 32, alignItems: 'start', cursor: 'none', transition: 'background 0.2s' }}
          ...
          >
            ...
            <div>
              <h3 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 24, color: '#f0f0f0', margin: '0 0 12px', lineHeight: 1.1 }}>{p.title}</h3>
...
function Certifications() {
  ...
  return (
    <section id="certifications" style={{ padding: '0 40px 120px', maxWidth: 1100, margin: '0 auto' }}>
      <SectionLabel n="04" text="CERTIFICATIONS" />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 0, border: '1px solid #1a1a1a' }}>
        {certs.map((c, i) => (
          <div key={i} style={{ padding: '28px 36px', borderBottom: i < certs.length - 1 ? '1px solid #161616' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24, cursor: 'none' }}>
...
function Contact() {
  ...
  return (
    <section id="contact" style={{ padding: '0 40px 80px', maxWidth: 1100, margin: '0 auto' }}>
